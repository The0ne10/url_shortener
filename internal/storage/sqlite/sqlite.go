...
func (s *Storage) SearchByURL(LoadURL string) (*string, error) {
	const op = "Storage.sqlite.SearchByURL"

	var alias string

	err := s.db.QueryRow("SELECT alias FROM urls WHERE url=?", LoadURL).Scan(&alias)
	if err != nil {
		return nil, fmt.Errorf("%s: %w", op, err)
	}
